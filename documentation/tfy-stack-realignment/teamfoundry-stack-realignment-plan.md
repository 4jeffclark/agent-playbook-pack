...
Program validation backs up from the earlier TradingCoach-first POC plan. Work proceeds in two passes—**Hello World**, then **TradingCoach**—each using the same three steps:

1. Materialize a simple but complete APP reference instance under `examples/`.
2. Baseline the APP standard in `standard/app-authoring.md`.
3. Validate reference instances against the standard.

Complete the Hello World pass before resuming TradingCoach.

| Item | Status |
| --- | --- |
| Hello World APP Example | Done |
| APP Standard (`standard/app-authoring.md`) | In Progress |
| Hello World Standard Conformance | Done |
| TradingCoach APP Example | In Progress |
| TradingCoach Execution Test | Not Started |
| TeamFoundry Employee Base Pilot | Paused |
| TFY Simulator | Deferred |

**Status values:** `Not Started`, `Paused` (started but blocked on an earlier pass), `In Progress`, `Done`, `Deferred`.
...
Do not record program coordination, documentation process, or pilot selection here. Those belong in **High-Level Status**, work patterns, or pillar planning notes.
...
3. Record materialization status in this plan document under **High-Level Status**.
...
- Do update **High-Level Status** in this plan when files are added.
...
3. Update `standard/app-authoring.md` and `app-design.md` only after acceptance.
...
1. Use [`README.md`](../../README.md) and `documentation/tfy-stack-realignment/README.md` as entry points.
...
2. Pack instance `README.md` is lightweight **user** documentation (welcome, examples, author notes). It may overlap manifest content in human-readable form but is not authoritative for execution.
3. Example READMEs in the workbench explain pack intent, source corpus, design sketches, and boundary tables where helpful for humans.
4. Status, current phase, materialization progress, next actions, and decisions live in the plan document.
5. Verify file existence on disk or in **High-Level Status** before assuming an example file exists.
...
Do not use this section for program process (status tracking, README rules, pilot selection, or documentation layout). Those live in **High-Level Status**, accepted work patterns, and pillar planning notes.
...
| 2026-06-23 | Accepted | **`agent-playbook-pack` is the APP standards workbench**, not a distribution repo. `examples/` holds reference `{packId}.app/` instances documenting the format alongside `docs/` and `schema/`. `examples/trading-coach.app/` is dev convenience during realignment and **graduates to its own distribution repo** after initial testing. | Supersedes treating `examples/` as a distribution repo; published repos remain `README.md` + `*.app/` only. |
...
| 2026-06-23 | Accepted | **Pack instance `README.md` is user documentation**, not execution agent code. Execution agents read `standard/app-authoring.md` (APP literacy), then pack manifests and referenced layer artifacts. | Supersedes APP-EXECUTION.md as pack shell entry. |
| 2026-06-23 | Accepted | **Gate metadata lives on `playbook.app.yaml`** as structured entries (`id`, `description`, optional `after` / `when`). Workflows clear gates by id; no separate `gates/` folder. | Single authoritative gate definition; hello-world reference updated; TradingCoach migration deferred. |
| 2026-06-23 | Accepted | **Workbench layout:** `standard/` (normative `app-authoring.md`), `examples/` (reference instances), `documentation/` (developer docs and TFY program). APP describes outcomes, not engine-specific bootstrap methods. | Supersedes `docs/` + `schema/` layout and per-pack `APP-EXECUTION.md` framework copy. |
| 2026-06-23 | Accepted | **APP is fire-and-forget** — behavioral instructions for execution agents only; no APP-owned execution tracking (run manifests, run records). Primary outputs and pack contracts define durable artifacts; post-run audit/history is consumer or user-datastore scope. | Supersedes structured run manifest as APP standard; removed from schema and hello-world reference. |
...
Validate APP as a portable behavior contract through two sequential passes. Each pass follows the three steps in **High-Level Status** (materialize example, baseline documentation, vanilla Cursor execution test).

**Hello World pass (current priority):**

1. Materialize a simple but complete `hello-world.app/` under `examples/`.
2. Baseline canonical agent-facing documentation from Hello World lessons.
3. Pass vanilla Cursor execution test (README with GitHub URL only).

**TradingCoach pass (paused until Hello World pass completes):**

1. ~~Translate the TradingCoach legacy project into a materialized `trading-coach.app/` instance~~ **Done** — `examples/trading-coach.app/` (regenerate: `documentation/tfy-stack-realignment/tools/migrate-trading-coach.py`).
2. Resume and reconcile documentation baseline using Hello World pass outcomes.
3. Pass vanilla Cursor execution test (README with GitHub URL only).

**Planning Notes:**

- Current baseline lives in `app-design.md`.
- APP standard: `standard/app-authoring.md`.
- General editorial references: `documentation/framework.md`, `documentation/naming.md`.
- Align reference instances to the standard; remove legacy pack shell files (`APP-EXECUTION.md`).
- Capture design lessons learned in this pillar after each pass completes; defer schema promotion and decision-log updates until then.

**Decisions:**

- See **APP Design Decision Log** (updates deferred until POC passes complete).

**Next Actions:**

- Align hello-world and trading-coach instances to `standard/app-authoring.md` (remove `APP-EXECUTION.md`, legacy gates/, etc.).
...
- Working sketch: `documentation/tfy-stack-realignment/sketches/teamfoundry-employee-base/README.md`.
...
- Initial consumption pilot follows the vanilla Cursor execution test in pillar 1 (TradingCoach pass).
- TFY employee-base migration paused; sketch at `documentation/tfy-stack-realignment/sketches/teamfoundry-employee-base/`.
