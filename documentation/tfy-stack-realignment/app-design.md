...
APP design should describe what APP owns as a framework: packs, playbooks, skill references, gates, overlays, contracts, outputs, compatibility metadata, examples, and consumption-facing expectations for TeamFoundry and other pack consumers.
...
- Graduate stable APP design into `standard/app-authoring.md`, `documentation/framework.md`, `examples/`, or adapter docs when it becomes durable.
...
An APP pack describes reusable assistant behavior and domain workflows in a runtime-neutral way. It defines what should happen for a user intent, which skills and workflows are involved, which gates must clear, which overlays may apply, and which outputs should result.
...
- `pack.app.yaml` — identity, pack-level inputs, playbook index
- instance `README.md` — lightweight user documentation
- referenced playbooks, skills, workflows, overlays, and contracts

Large monolithic execution constitutions (legacy `PROJECT.md` style) decompose into workflows and contracts—not a mandatory multi-document read chain before execution.

At minimum, each instance should make discoverable:

- pack and playbook manifests
- local skills in `layer1-skills/` (agentskills.io format), workflows, overlays, and contracts referenced by those manifests
- enough entry guidance for an agent or runtime to find packs, resolve playbooks, and begin execution via `pack.app.yaml` and manifests
...
- durable artifacts named by contracts (reports, snapshots, domain records, etc.)
...
| Bootstrap requires long doc chain (`PROJECT.md`, git protocol, etc.) | `standard/app-authoring.md` + `pack.app.yaml` + referenced manifests, workflows, and contracts |
...
Provisional manifest shape (POC; see `standard/app-authoring.md`):
...
| Outputs | Primary artifacts declared on playbook manifests and output contracts |
...
Gate metadata is declared on each `playbook.app.yaml` (no separate `gates/` folder).
...
| `README.md` | Lightweight user documentation (welcome, examples, author notes); not authoritative for execution |
...
Outputs describe the primary artifacts a playbook produces, using logical role path templates. Output shape and required files are defined in pack `contracts/` referenced from manifests.
...
| General APP framework docs | `documentation/framework.md` |
| Naming notes | `documentation/naming.md` |
