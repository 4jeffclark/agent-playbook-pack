...
def write_pack_manifest() -> None:
    ...
    intro += (
        ...
        "`playbook: <id>`), and optional partial `inputs`. See `pack.app.yaml` and pack contracts.\n\n"
        "---\n\n"
...
def write_examples_readme() -> None:
    text = (
        ...
        "Reference `{packId}.app/` instances — side-by-side helpers for "
        "[`../standard/app-authoring.md`](../standard/app-authoring.md).\n\n"
        "This folder is **not** an APP distribution repo.\n\n"
        "## Instances\n\n"
        "| Instance | Role |\n"
        "| --- | --- |\n"
        "| [hello-world.app/](hello-world.app/) | Minimal reference — standard layer shapes |\n"
...
def main() -> None:
    ...
    tools = REPO_ROOT / "documentation" / "tfy-stack-realignment" / "tools"
    subprocess.run(
        [sys.executable, str(tools / "baseline-app-skills.py")],
        check=True,
    )
    subprocess.run(
        [sys.executable, str(tools / "materialize-skill-scripts.py")],
        check=True,
    )
