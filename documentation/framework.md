...
APP concepts and layer model. Workbench entry: [`../README.md`](../README.md). Normative standard: [`../standard/app-authoring.md`](../standard/app-authoring.md).
...
APP is **fire-and-forget**: it supplies behavioral instructions for downstream execution agents and has no ongoing involvement in runs after outputs are written. Execution tracking is out of scope.
...
6. Support durable primary output artifacts where useful.
...
- An execution tracking or audit system
...
Cross-cutting pack folder `contracts/` is not layer-numbered. Gate definitions live on each `playbook.app.yaml`. Each pack lives in a self-contained `{packId}.app/` folder. APP distribution repos contain only `README.md` and `*.app/` at repo root.

Normative layout and execution outcomes: [`../standard/app-authoring.md`](../standard/app-authoring.md).
