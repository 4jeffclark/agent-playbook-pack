...
2. Baseline canonical agent-facing documentation.
3. Run an end-to-end execution test.

Complete the Hello World pass before resuming TradingCoach.

**Execution test harness:** Initial end-to-end runs use a **vanilla Cursor project** with no Standards Workbench context copied in. Bootstrap is a single `README.md` containing the **Standards Workbench** GitHub URL and an instruction to execute a pack under `examples/` (for example `examples/hello-world.app/`). The test agent must run from the pack instance's `APP-EXECUTION.md` without treating this repo as a distribution repo.

| Item | Status |
| --- | --- |
| Hello World APP Example | In Progress |
| APP Standards Documentation | In Progress |
| Hello World Execution Test | Not Started |
| TradingCoach APP Example | Paused |
...
### Vanilla Cursor Execution Test Pattern

**Status:** Accepted

**Intent:** Validate that APP distribution repos are self-sufficient for execution agents without workbench or TFY context.

**Pattern:**

1. Materialize the target pack under `examples/` in the APP repo (`examples/README.md` + `*.app/` only).
2. Baseline canonical agent-facing documentation (`docs/app-execution.md`, `docs/app-skills.md`, per-instance `APP-EXECUTION.md`).
3. Create a separate vanilla Cursor project whose only bootstrap is a `README.md` with the **Standards Workbench** GitHub URL and an instruction to execute a pack under `examples/{packId}.app/`.
4. Run the test agent cold; record gaps in documentation or pack layout before advancing to the next example pass.

**Guardrails:**

- Do not copy framework docs, plan context, or workbench files into the test project.
- Do not use the TFY simulator or in-repo reference executor for these initial passes.
- Do not advance to TradingCoach until the Hello World pass completes all three steps.
- Do update **High-Level Status** when a pass step completes.
...
1. Use [`README.md`](../../README.md) and `docs/tfy-stack-realignment/README.md` as entry points.
...
4. Verify file existence on disk or in **High-Level Status** before assuming an example file exists.
...
1. ~~Translate the TradingCoach legacy project into a materialized `trading-coach.app/` instance~~ **Done** — `examples/trading-coach.app/` (regenerate: `docs/tfy-stack-realignment/tools/migrate-trading-coach.py`).
...
- Agent-facing docs to baseline: `docs/app-execution.md`, `docs/app-skills.md`, per-instance `APP-EXECUTION.md`.
- General APP framework references remain in `docs/framework.md`, `docs/app-skills.md`, and `schema/app-manifest-v0.1.md` until design content graduates.
...
- Complete Hello World documentation baseline; reconcile TradingCoach `APP-EXECUTION.md` to the instance copy pattern.
...
- Working sketch: `docs/tfy-stack-realignment/sketches/teamfoundry-employee-base/README.md`.
...
- TFY employee-base migration paused; sketch at `docs/tfy-stack-realignment/sketches/teamfoundry-employee-base/`.
